...
class LinearSolver(object):
    ...
    def getA(self,PAI, B, W):
        ...
        try:
            data = np.exp(-safe_sparse_dot(B, W).data)
            A = sp.csr_matrix(PAI * data)
    ...
    def updateW(self,X_uv, X_uf, epoc, Z, batch_size, a_1, eta):
        ...
        W = sp.csr_matrix(np.random.uniform(low=-0.5 / d_dim, high=0.5 / d_dim, size=d_dim))
        W = W.T
        p_start = 0
        p_end = batch_size
